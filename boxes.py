...
class BoxesGame(ConnectionListener):
    ...
    def __init__(self):
        ...
        self.num=None
        #connect by network
        address=raw_input("Address of Server:(X:X)")
        try:
            if not address:
        ...
        while not self.running:
            ...
            connection.Pump()
            sleep(0.01)
        #determin attributes from player
        if self.num==0:
            self.turn=True
    ...
    def update(self):
        if self.me+self.otherplayer==36:
            self.didiwin=True if self.me>self.othermarker else False
            return 1
        self.justplaced-=1
        connection.Pump()
        self.Pump()
